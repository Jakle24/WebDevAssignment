from fastapi import FastAPI, HTTPException, Depends, status
from fastapi.security import OAuth2PasswordRequestForm
from pydantic import BaseModel
from typing import List, Optional
import mysql.connector
from auth import AuthHandler
from schemas import AuthDetails

app = FastAPI()
auth_handler = AuthHandler()

# Database connection
def get_db():
    return mysql.connector.connect(
        host="localhost",
        user="your_user",
        password="your_password",
        database="sakila"
    )

# Pydantic Models
class Film(BaseModel):
    film_id: int
    title: str
    description: Optional[str]
    release_year: int

class NewCustomer(BaseModel):
    store_id: int
    first_name: str
    last_name: str
    email: str
    address_id: int
    active: int

class UpdateAddress(BaseModel):
    address_id: int
    address: str
    district: str

# --- AUTH ---
@app.post("/token", tags=["Token"])
def login(auth_details: AuthDetails):
    if not auth_handler.authenticate(auth_details.username, auth_details.password):
        raise HTTPException(status_code=401, detail="Invalid credentials")
    token = auth_handler.encode_token(auth_details.username)
    return {"token": token}

# --- GET ENDPOINTS ---

@app.get("/films", response_model=List[Film])
def get_all_films():
    db = get_db()
    cursor = db.cursor()
    cursor.execute("SELECT film_id, title, description, release_year FROM film LIMIT 10")
    results = cursor.fetchall()
    cursor.close()
    return [Film(film_id=f[0], title=f[1], description=f[2], release_year=f[3]) for f in results]

@app.get("/films/category/{category_id}", response_model=List[Film])
def get_films_by_category(category_id: int):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("""
        SELECT f.film_id, f.title, f.description, f.release_year
        FROM film f
        JOIN film_category fc ON f.film_id = fc.film_id
        WHERE fc.category_id = %s
    """, (category_id,))
    results = cursor.fetchall()
    cursor.close()
    if not results:
        raise HTTPException(status_code=404, detail="No films found in this category")
    return [Film(film_id=f[0], title=f[1], description=f[2], release_year=f[3]) for f in results]

@app.get("/customers/active/{store_id}", response_model=List[NewCustomer])
def get_active_customers(store_id: int):
    db = get_db()
    cursor = db.cursor(dictionary=True)
    cursor.execute("SELECT * FROM customer WHERE store_id = %s AND active = 1", (store_id,))
    return cursor.fetchall()

# --- POST ENDPOINTS ---

@app.post("/customers/new", status_code=201)
def create_customer(customer: NewCustomer):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("""
        INSERT INTO customer (store_id, first_name, last_name, email, address_id, active, create_date)
        VALUES (%s, %s, %s, %s, %s, %s, NOW())
    """, (customer.store_id, customer.first_name, customer.last_name, customer.email, customer.address_id, customer.active))
    db.commit()
    cursor.close()
    return {"message": "Customer created successfully"}

@app.post("/films/new", status_code=201)
def add_new_film(title: str, description: str, release_year: int, language_id: int):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("""
        INSERT INTO film (title, description, release_year, language_id)
        VALUES (%s, %s, %s, %s)
    """, (title, description, release_year, language_id))
    db.commit()
    cursor.close()
    return {"message": "Film added successfully"}

# --- PUT ENDPOINTS ---

@app.put("/address/update", status_code=200)
def update_customer_address(data: UpdateAddress):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("UPDATE address SET address=%s, district=%s WHERE address_id=%s",
                   (data.address, data.district, data.address_id))
    db.commit()
    cursor.close()
    return {"message": "Address updated"}

@app.put("/film/title/{film_id}", status_code=200)
def update_film_title(film_id: int, title: str):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("UPDATE film SET title=%s WHERE film_id=%s", (title, film_id))
    db.commit()
    cursor.close()
    return {"message": "Film title updated"}

# --- DELETE ENDPOINTS ---

@app.delete("/customer/delete/{customer_id}", status_code=200)
def delete_customer(customer_id: int):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("DELETE FROM customer WHERE customer_id=%s", (customer_id,))
    db.commit()
    cursor.close()
    return {"message": "Customer deleted"}

@app.delete("/film/delete/{film_id}", status_code=200)
def delete_film(film_id: int):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("DELETE FROM film WHERE film_id=%s", (film_id,))
    db.commit()
    cursor.close()
    return {"message": "Film deleted"}
